...
void App::onTwitterTimeline(QString info, bool success)
{
    if (success)
    {
        switch (m_viewType)
        {
            case Rich:
            {
                QmlDocument* qml = QmlDocument::create("asset:///CustomTimelineView.qml").parent(this);
                qml->setContextProperty("app", this);
                Page* root = qml->createRootObject<Page>();
                m_navPane->push(root);
            }
            ...
            default:
            {
                QmlDocument* qml = QmlDocument::create("asset:///StandardTimelineView.qml").parent(this);
                qml->setContextProperty("app", this);
                Page* root = qml->createRootObject<Page>();
                m_navPane->push(root);
            }
...
void App::parseResponse()
{
    ...
    else if (m_viewType == App::Rich)
    {
        ...
        list->setVisible(true);
        list->setDataModel(dm);
        list->setListItemProvider(tweetItemMgr);
    }
}
